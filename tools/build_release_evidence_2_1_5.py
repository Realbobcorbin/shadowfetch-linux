#!/usr/bin/env python3
"""Build reproducible Shadowfetch Linux 2.1.5 release evidence."""
...
def build_dossier(manifest: dict[str, object], timestamp: str) -> str:
    ...
    lines = [
        ...
        "## Release intent",
        "",
        "Shadowfetch Linux 2.1.5 keeps the Fire Edition and Umbra visual identity while making the first Linux session more useful and more honest. Shadowfetch Guide and its private System Passport explain what works without uploading machine identity. Buzz is the single optional local-AI workspace, recommends an open model only after consent, and keeps its relay and compute listeners loopback-only by default. Codex, Claude Code, Grok Build, and Cursor Agent are independent, verified, user-owned options with no credentials preloaded.",
        "",
